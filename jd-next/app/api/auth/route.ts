  ...
  try {
    ...
    if (action === "register") {
      ...
      const newUsers = await query<{ id: string; name: string; role: string }>(
        ...
        [name, phone, email || null, role, hash]
      );

      const newUser = newUsers[0];

      // For farmers: also create a farm record
      if (role === "farmer") {
        const { district, village, land_acres, crops, storage } = body;
        try {
          await query(
            `INSERT INTO farms (farmer_id, farm_name, district, village, total_area_acres, storage_location, jeevadhara_certified, created_at)
            ...
            [
              ...
              district || "Telangana",
              village || "",
              land_acres ? parseFloat(land_acres) : null,
              storage || "",
            ]
          );
        } catch (farmErr) {
          console.error("Farm creation error (non-fatal):", farmErr);
        }
      }
